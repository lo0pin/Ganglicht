...
/* ---------------------------- Funktionsköpfe ----------------------------- */
/*  Hinweis: Einige Prototypen (z. B. setleft/setright/convert_led) werden
    in dieser konkreten Version nicht verwendet, bleiben jedoch als Schnittstelle
    bestehen und ändern keine Funktionalität. */
void setleft();
void setright();
void shutoff();
void keepon();
...
/* -------------------- Platzhalter/Prototypen (ungenuzt) ------------------ */
/*
  Folgende Prototypen bleiben aus Kompatibilitätsgründen erhalten, werden
  jedoch in dieser konkreten Fassung nicht aufgerufen. Eine Definition ist
  deshalb nicht erforderlich.
*/
void setleft();   // nicht verwendet
void setright();  // nicht verwendet
int  convert_led(int led, int sensorbool); // nicht verwendet
